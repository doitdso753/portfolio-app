...
const setNavbarMenuOpen = (isOpen) => {
  navbarMenu.classList.toggle('open', isOpen);
  navbarToggleBtn.setAttribute('aria-expanded', String(isOpen));
  navbarToggleBtn.setAttribute('aria-label', isOpen ? '메뉴 닫기' : '메뉴 열기');
};
...
navbarToggleBtn.addEventListener('click', () => {
  setNavbarMenuOpen(navbarToggleBtn.getAttribute('aria-expanded') !== 'true');
});
...
skillItems.forEach((item) => {
  ...
  header.addEventListener('click', () => {
    ...
    skillItems.forEach((skillItem) => {
      ...
      skillItem.querySelector('.skill-item__content').setAttribute('aria-hidden', 'true');
    });

    if (!isOpen) {
      item.classList.add('open');
      header.setAttribute('aria-expanded', 'true');
      item.querySelector('.skill-item__content').setAttribute('aria-hidden', 'false');
    }
  });
});
